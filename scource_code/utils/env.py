...
class ConnectFour:
    def __init__(self, agent:Any, board_shape=(7,6)):
        self.ai_on = True
        self.agent = agent
        self.depth = 1
        self.score = {1: 0, 2: 0}

        self.init_window(board_shape)
        self.init_menu()

    def init_window(self, board_shape):
        pg.init()
    ...
    @staticmethod
    def draw_text(surface, text, size, color, x, y) -> None:
    ...
    def change_board_size(self, value: tuple[str,int], size_n=int) -> str:
        selected = value[0]
    ...
    def get_action(self, observation:np.ndarray) -> int:
        ...
        if ((self.player == 1) or (self.player == 2 and not self.ai_on)) and self.b_down:
            ...
            action = self.agent.minimax(observation, self.depth, True)[0]
